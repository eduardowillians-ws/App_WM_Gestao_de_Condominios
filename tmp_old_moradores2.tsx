...
const INITIAL_RESIDENTS: Resident[] = [
  ...
  { id: '2', name: 'Jo├úo Silva', blockId: 'b1', unit: '202', email: 'joao@email.com', phone: '(11) 99999-0002', status: 'active', rg: '23.456.789-0', cpf: '234.567.890-11', profession: 'Arquiteto', entryDate: '2022-05-20', photoUrl: 'https://i.pravatar.cc/150?u=joao' },
...
const PROFESSIONS = ['Engenheiro(a)', 'Arquiteto(a)', 'Advogado(a)', 'M├®dico(a)', 'Aut├┤nomo(a)', 'Empres├írio(a)', 'Professor(a)', 'Designer', 'Desenvolvedor(a)', 'Aposentado(a)'];
...
const Moradores: React.FC = () => {
  ...
  // Estados de Formul├írio Controlados para M├íscaras
  const [formRG, setFormRG] = useState('');
  const [formCPF, setFormCPF] = useState('');
  const [formPhone, setFormPhone] = useState('');

  // M├íscaras em tempo real
  ...
  // L├│gica de Filtros Hier├írquicos
  ...
  const handleSaveResident = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!editingResident) {
        ...
        // Isso n├úo vai deslogar o Admin!
        ...
        // NOVO C├ôDIGO: Verifica se a Edge Function retornou erro mascarado no data 
        // (ela agora retorna 200 para n├úo quebrar a tela, por├®m envia success: false com o texto real do erro)
        if (edgeData && edgeData.success === false) {
           throw new Error(edgeData.error || 'Erro desconhecido ao tentar convidar.');
        }
        
        alert('Convite enviado com sucesso! O morador receber├í um e-mail para definir a senha e preencher o restante do perfil.');
      } else {
        // L├│gica de Edi├º├úo de Morador Existente pelo Admin
  ...
  const handleDeleteResident = async (id: string) => {
    if (!window.confirm('Tem certeza que deseja remover este morador?\n\nEle sai do app mas os dados ficam salvos para relat├│rios.')) return;
    
    setActiveMenu(null);
    
    // Soft delete - mant├®m no banco mas marca como inativo
    try {
      const { error } = await supabase
        .from('profiles')
        .update({ 
      ...
      alert('Morador removido. Dados salvos para hist├│rico.');
    } catch (err) {
      console.error('Erro ao remover:', err);
      // Fallback local se banco n├úo responder
      setResidents(prev => prev.map(r => 
        r.id === id ? { ...r, status: 'inactive' as const } : r
      ));
  ...
  const handleTransferResident = async (resident: Resident) => {
    ...
    // Verifica se a unidade j├í est├í ocupada
    const occupied = residents.find(r => r.blockId === newBlockId && r.unit === newUnit && r.status === 'active');
    if (occupied) {
      alert('Esta unidade j├í est├í ocupada por ' + occupied.name);
      return;
    }
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">Gest├úo da planta do condom├¡nio e base de dados de residentes.</p>
        </div>
...
<button 
      ...
      {activeTab === 'list' ? (
        <>
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="flex flex-col">
                <input type="text" placeholder="N┬║ Apartamento" value={filterUnit} onChange={(e) => setFilterUnit(e.target.value)} className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 font-black text-slate-700 shadow-inner" />
              </div>
            </div>
          </div>

          {/* Listagem com Menu de A├º├Áes Corrigido */}
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100 font-black text-[10px] text-gray-400 uppercase tracking-widest">
                    ...
                    <th className="px-10 py-6 text-right">A├º├Áes</th>
                  </tr>
                </thead>
        ...
        /* VIS├âO DE MAPA (SEM ALTERA├ç├òES) */
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
              ...
              <div className="p-10 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">N├║mero do Apartamento</label>
                  <input 
                    id="newUnit" 
                ...
                <div className="bg-yellow-50 p-4 rounded-2xl border border-yellow-100">
                  <p className="text-[10px] text-yellow-700 font-bold">
                    ...
                    O morador atual ser├í <span className="uppercase">removido</span> e um novo cadastro ser├í criado na nova unidade.
                  </p>
                </div>
      ...
      {/* MODAL BLOCO (SEM ALTERA├ç├òES) */}
      {isBlockModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md" onClick={() => setIsBlockModalOpen(false)}></div>
          <div className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="px-10 py-8 mycond-bg-blue text-white flex justify-between items-center">
              <div><h3 className="text-2xl font-black uppercase tracking-tighter">Configurar Unidade</h3><p className="text-[10px] text-blue-200 font-bold uppercase">Arquitetura do Condom├¡nio</p></div>
      ...
      {/* MODAL MORADOR (COM L├ôGICA DE EDI├ç├âO) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
            ...
            <form onSubmit={handleSaveResident} className="p-10 space-y-6 max-h-[75vh] overflow-y-auto scrollbar-hide">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">N┬║ Unidade</label>
              ...
              <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{editingResident ? 'Nome Completo' : 'Respons├ível pela Unidade'}</label>
              ...
              {/* CAMPOS RESTRITOS A EDI├ç├âO (PREENCHIMENTO INICIAL APENAS NOME E UNIDADE E EMAIL) */}
              {editingResident && (
                <>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Profiss├úo</label>
              ...
              <button type="submit" className="w-full py-6 mycond-bg-yellow rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl">
                {editingResident ? 'Salvar Altera├º├Áes' : 'Finalizar Cadastro'}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[4rem] shadow-2xl p-10 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto scrollbar-hide">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                <div className="p-8 bg-slate-50 rounded-[2.5rem] space-y-4 shadow-inner">
                   <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b pb-2">Informa├º├Áes Pessoais</p>
                   <div className="flex justify-between"><span className="text-[9px] font-black text-gray-400 uppercase">RG</span><span className="text-[11px] font-black text-slate-800">{viewingResident.rg || 'N├úo informado'}</span></div>
                   <div className="flex justify-between"><span className="text-[9px] font-black text-gray-400 uppercase">CPF</span><span className="text-[11px] font-black text-slate-800">{viewingResident.cpf || 'N├úo informado'}</span></div>
                   <div className="flex justify-between"><span className="text-[9px] font-black text-gray-400 uppercase">Profiss├úo</span><span className="text-[11px] font-black text-slate-800 uppercase">{viewingResident.profession || 'N/I'}</span></div>
                </div>
                <div className="p-8 bg-slate-50 rounded-[2.5rem] space-y-4 shadow-inner">
