...
const Moradores: React.FC = () => {
  ...
  const handleSaveBlock = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const name = toTitleCase(formData.get('blockName') as string);
...
<button 
                ...
                <>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                       ...
                       <input required name="email" type="email" defaultValue={editingResident?.email} readOnly={!!editingResident} className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 font-black text-slate-800 shadow-inner" placeholder="Ex: morador@email.com" />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                       ...
                       {(() => {
                         ...
                         if (availableUnits.length > 0) {
                           return (
                             <select name="unit" required defaultValue={editingResident?.unit || ""} className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 font-black text-slate-800 shadow-inner">
                               <option value="">Selecione...</option>
      ...
      {showInvitesPanel && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[85vh]">
            <div className="px-10 py-8 mycond-bg-blue text-white flex justify-between items-center shrink-0">
               <h3 className="text-2xl font-black uppercase tracking-tighter">
                 <i className="fa-solid fa-paper-plane mr-3 text-blue-200"></i>
                 Convites Enviados
               </h3>
               <button onClick={() => setShowInvitesPanel(false)} className="w-12 h-12 rounded-full hover:bg-white/10 flex items-center justify-center transition-colors">
